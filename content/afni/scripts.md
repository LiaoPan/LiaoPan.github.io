---
title: "AFNI 系列教程 #3.脚本批量运行"
date: 2023-07-21T17:04:34+08:00
draft: true
weight: 3
pre: <b>3. </b>
---
...
- [用户可以指定一个运行被试](#用户可以指定一个运行被试)
    - [参考资料](#参考资料)


### 概述

在对单个受试者的单次运行进行预处理并建立模型后，你需要对数据集中所有受试者的所有运行执行相同的步骤。这看似繁琐，但还是可以做到的--我们只有 26个研究对象，每个研究对象有两次run。你可能认为一周左右的时间就能完成，而且还可以将这项任务分配给几位研究助理。

这种态度是值得钦佩的，如果你采取这种方法，你最终将能够分析所有数据。但在某些时候，你会遇到两个问题：

1. 你会发现手动分析每次运行不仅乏味，而且容易出错，而且随着要分析的运行次数增加，出错的概率也会大大增加；
2. 对于较大的数据集（例如，80 个受试者，每个受试者有 5 次run），这种方法很快就会变得不切实际。
   
另一种方法是编写分析脚本。就像演员有一个脚本，告诉他该说什么、站在哪里、在哪里移动一样，你也可以编写一个脚本，告诉计算机如何分析数据集。这样做有双重好处，一是可以实现分析自动化，二是可以分析任何规模的数据集--分析两个受试者或两百个受试者的代码几乎完全相同。

首先，我们将创建一个模板，其中包含分析单次运行所需的代码，然后使用 for 循环自动分析所有运行。这个想法很简单；虽然代码一开始可能难以理解，但一旦你熟悉了它，你就会明白如何将它应用于任何数据集。


### 创建模板
在分析 sub-08 的第一次运行时，你创建了一个名为 proc.sub_08 的文件。该文件包含一个 AFNI 命令列表，其组成方式由 uber_subject.py GUI 决定。将该文件复制到包含所有被试的目录中，并重命名为 proc_Flanker.sh。例如，如果 proc.sub_08 脚本位于由 uber_subject.py 生成的 sub-08 目录结构中，则可以在包含所有被试的目录中键入以下内容：
```
...
1. 我们将删除所有对 sub-08 的引用，并将这些字符串转化为一个变量，该变量取自给定给脚本的参数。例如，我们将修改脚本，这样如果我们输入`tcsh proc_Flanker.sh sub-01`执行脚本，它就会用 sub-01 字符串替换脚本中的变量，并分析该被试的数据。
2. 我们将替换路径，使其更具通用性。

首先，在文本编辑器中打开 proc_Flanker.sh 文件。滚动到第 26-31 行，其中包含以下代码：
...
接下来，我们需要用相对路径替换绝对路径。我们将用 $PWD 变量替换它，它是当前工作目录路径的简称。这将确保脚本适应当前计算机的目录结构，并且不会因脚本无法找到某些文件的位置而出错。


所有的模板代码可以直接查看下文或者[在github上进行下载](https://github.com/andrewjahn/AFNI_Scripts/blob/master/proc_Flanker.sh)。
{{< tabs >}}
{{% tab name="proc_Flanker.sh" %}}
```shell
#!/bin/tcsh -xef
...
{{% notice note %}}
为了加快分析速度，我更喜欢在 3dDeconvolve 命令中使用 -mask 选项。例如，我会将脚本的第 299 行改为`3dDeconvolve -input pb04.$subj.r*.scale+tlrc.HEAD -mask mask_group+tlrc`。我们有理由反对这样做，例如，大脑之外可能存在系统性变化，而屏蔽掉非脑体素将会错过这些变化。不过，使用掩码可以大大加快回归块的处理速度；而且我认为，如果在大脑之外存在任何 "问题"体素，通过检查每个预处理块的输出结果就能发现它们。
{{% /notice %}}


### 自动分析
现在，我们将在 for 循环中使用更新后的预处理脚本来分析数据集中的所有受试者。请使用以下代码
```shell
...
```
这将针对文件 "subjList.txt "中的每个被试运行脚本 "proc_Flanker.sh"，脚本每次运行时都会使用 subjList.txt 文件中的每一行作为参数。

这将对每个研究对象进行预处理和回归，并将输出结果存储在名为`<subjID>/<subjID>.results`的文件夹中，其中 "subjID"代表研究对象名称。每次分析需要 5-10 分钟，这取决于你的电脑速度。


### 参考资料
- https://andysbrainbook.readthedocs.io/en/latest/AFNI/AFNI_Short_Course/AFNI_06_Scripting.html